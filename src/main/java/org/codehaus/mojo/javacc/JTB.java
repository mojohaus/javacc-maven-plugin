...
class JTB extends ToolFacade {
    ...
    /**
     * Whether to generate the ObjectDepthFirst convenience visitor class.
     */
    ...
    /**
     * Sets whether to generate the ObjectDepthFirst convenience visitor class.
     *
     * @param value If <code>true</code>, an ObjectDepthFirst visitor class will be generated that extends
     *            GJDepthFirst&lt;Object, Object&gt;, may be <code>null</code>.
     */
    public void setGenerateObjectDepthFirst(Boolean value) {
        this.generateObjectDepthFirst = value;
    }
    ...
    protected int execute() throws Exception {
        ...
        if (exitcode == 0 && this.generateObjectDepthFirst != null && this.generateObjectDepthFirst) {
            generateObjectDepthFirstVisitor();
        }
    ...
    /**
     * Generates the ObjectDepthFirst convenience visitor class that extends GJDepthFirst&lt;Object, Object&gt;.
     * This provides a non-generic visitor class for users who prefer to work with Object types.
     *
     * @throws IOException If the file generation failed.
     */
    private void generateObjectDepthFirstVisitor() throws IOException {
        File visitorDir = getEffectiveVisitorDirectory();
        if (visitorDir == null) {
            getLog().warn("Cannot generate ObjectDepthFirst: visitor directory not set");
            return;
        }

        File objectDepthFirstFile = new File(visitorDir, "ObjectDepthFirst.java");
        String visitorPackage = getEffectiveVisitorPackageName();
        String nodePackage = getEffectiveNodePackageName();

        StringBuilder content = new StringBuilder();
        content.append("//\n");
        content.append("// Generated by javacc-maven-plugin\n");
        content.append("//\n\n");

        if (visitorPackage != null && !visitorPackage.isEmpty()) {
            content.append("package ").append(visitorPackage).append(";\n");
        }

        content.append("import ").append(nodePackage).append(".*;\n");
        content.append("import java.util.*;\n\n");

        content.append("/**\n");
        content.append(" * Provides default methods which visit each node in the tree in depth-first\n");
        content.append(" * order.  Your visitors may extend this class.\n");
        content.append(" * <p>\n");
        content.append(" * This is a convenience class that extends GJDepthFirst&lt;Object, Object&gt; and\n");
        content.append(" * provides a non-generic interface for visitors that work with Object types.\n");
        content.append(" */\n");
        content.append("public class ObjectDepthFirst extends GJDepthFirst<Object, Object> {\n");
        content.append("}\n");

        try {
            FileUtils.fileWrite(objectDepthFirstFile, "UTF-8", content.toString());
            getLog().info("Generated ObjectDepthFirst visitor: " + objectDepthFirstFile);
        } catch (IOException e) {
            getLog().error("Failed to generate ObjectDepthFirst visitor: " + e.getMessage());
            throw e;
        }
    }
