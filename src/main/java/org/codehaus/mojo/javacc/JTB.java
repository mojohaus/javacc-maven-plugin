...
class JTB extends ToolFacade {
     ...
     * files in the target directory will be overwritten. This method recursively handles subdirectories to support
     * JTB 1.5.x signature packages.</p>This method must be used instead of
    ...
    private void moveDirectory(File sourceDir, File targetDir) throws IOException {
        ...
        for (File sourceFile : sourceFiles) {
            if (sourceFile.isFile() && sourceFile.getName().endsWith(".java")) {
                ...
                // Recursively handle subdirectories (e.g., signature package in JTB 1.5.x)
                File targetSubDir = new File(targetDir, sourceFile.getName());
                moveDirectory(sourceFile, targetSubDir);
            }
        }
